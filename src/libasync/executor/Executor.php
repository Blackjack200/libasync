...
final class Executor extends Thread implements AsyncRuntime {
	...
	private function runTasks(...$argsInjected) : void {
		$runCount = 0;
		while (!$this->isKilled && $this->queue->synchronized(fn() => $this->queue->count()) > 0) {
			$runCount++;
			[$rec, $closure, $extraArgPrepareFunc, $extraArgDestroyFunc] = $this->queue->synchronized(fn() => $this->queue->shift());
